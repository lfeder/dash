...
(function (global) {
  ...
  function fetchSheetGviz(sheetId, gid) {
    return new Promise((resolve, reject) => {
      const cbName = '_cb' + gid + '_' + Math.floor(Math.random() * 1e9);
      ...
      const script = document.createElement('script');
      script.src = url;
      script.onerror = () => { delete global[cbName]; reject('Network error loading gid=' + gid); };
      document.head.appendChild(script);
    });
  }
  ...
  async function fetchTable(logicalName, opts = {}) {
    ...
    if (mode === 'sheets') {
      ...
      const tables = await Promise.all(conf.sheets.map(s => fetchSheetGviz(s.sheetId, s.gid)));
