...
(function (global) {
  ...
  const CONFIG = {
    ...
    lettuce_harvest: {
      sheets: [
        // The sheet's grow_L_seeding tab carries both seeding config and
        // harvest outcome per cycle. For dashboards that just need harvest
        // rollups (boards, pounds), the grow_lettuce_harvest_v view below
        // reconstructs the same shape from grow_harvest_weight.
        { sheetId: SHEETS.grow, tab: 'grow_L_seeding' },
      ],
      supabase: {
        table: 'grow_lettuce_harvest_v',
        select: '*',
        columns: [
          { label: 'HarvestDate',         field: 'harvest_date',           type: 'date'   },
